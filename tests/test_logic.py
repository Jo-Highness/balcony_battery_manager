...
def test_rest_pv_energy_afternoon_cap():
    ...
    sunset = datetime(2026, 7, 1, 17, 0, tzinfo=UTC)  # 3 h
...
def test_charge_release_deficit():
    ...
    sunset = datetime(2026, 7, 1, 17, 0, tzinfo=UTC)  # 1 h left
...
def test_charge_release_pv_sufficient():
    ...
    sunset = datetime(2026, 7, 1, 20, 0, tzinfo=UTC)  # 10 h, morning full cap
    rel, _need, _rest = logic.charge_release(
        now,
        sunset,
        soc=95,
        p_anker_pv=1800,
...
def test_grid_support_needs_solarbank_charge():
    a = logic.grid_support_active(
        False, main_soc=5, soc_anker=8, empty_soc=10, soc_hyst=3, min_discharge_soc=10, enabled=True
    )
...
def test_grid_support_disabled_or_no_main_soc():
    assert (
        logic.grid_support_active(
            False, 5, 50, empty_soc=10, soc_hyst=3, min_discharge_soc=10, enabled=False
        )
        is False
    )
    assert (
        logic.grid_support_active(
            False, None, 50, empty_soc=10, soc_hyst=3, min_discharge_soc=10, enabled=True
        )
        is False
    )
